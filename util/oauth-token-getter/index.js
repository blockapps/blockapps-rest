...
app.get(callbackPath, async function (req, res) {
  try {
    ...
    const tokenField = oauthConfig.tokenField ? oauthConfig.tokenField : 'access_token';
    res.send(`Copy your ACCESS_CODE: <br/><br/>${accessTokenResponse.token[tokenField]}<br/><br/>\Code expires in: ~${Math.floor(accessTokenResponse.token['expires_in'] / 60)} min`)
  } catch(err) {
