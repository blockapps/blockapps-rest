...
function* createUser(name, password, isAsync, node) {
  ...
  const txResult = yield fill(user, {resolve: true, node: node});
  return user;
}
...
function* fill(user, options) {
  ...
  }, user.name, user.address, options.resolve, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return blocResult;
...
function* getBlocResults(hashes, options) {
  ...
  const result = yield api.bloc.results(hashes, options.resolve, options.chainId, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
function *  uploadContractListOAuth(accessToken, txs, options){
  options = options || {};
  const resolve = options.resolve;
  verbose('uploadContractListOAuth', {accessToken, txs, options, resolve})
