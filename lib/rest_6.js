...
function constructMetadata(options, contractName) {
  ...
  // {history: ["Contract1", "Contract2"]}
  if (options.hasOwnProperty('history')) {
    var newContracts = '';
    options['history'].forEach(function(contract) {
      if (contract !== contractName) {
        var newContract = ',' + contract;
        newContracts += newContract;
      }
    });
    const currentHistory = options['history'];
    const newHistory = currentHistory + newContracts;
    metadata['history'] = newHistory;
  }
  ...
  // {noindex: ["Contract1", "Contract2"]}
  if (options.hasOwnProperty('noindex')) {
    var newContracts = '';
    options['noindex'].forEach(function(contract) {
      if (contract !== contractName) {
        var newContract = ',' + contract;
        newContracts += newContract;
      }
    });
    const currentNoindex = options['noindex'];
    const newNoindex = currentNoindex + newContracts;
    metadata['noindex'] = newNoindex;
  }
...
function* call(user, contract, methodName, args, value, options, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('call', {user, contract, methodName, args, value, options, resolve, chainId, node});
  ...
  options = options || {};
  const metadata = constructMetadata(options, contract.name);
  ...
  var result = yield api.bloc.call({
      ...
      value: valueFixed,
      metadata: metadata
    }, user.name, user.address, contract.name, contract.address, resolve, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  ...
  if(resolve) {
    ...
    result = yield resolveResult(result, chainId, node);
...
function* callMethod(user, contract, methodName, args, value, options, doNotResolve, chainId, node) {
  return yield call(user, contract, methodName, args, value, options, doNotResolve, chainId, node);
}
...
*/
function* callList(user, txs, options, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  ...
  const results = yield api.bloc.callList({
      ...
      txs: txs,
      resolve: resolve,
    }, user.name, user.address, resolve, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  // verity results
  ...
  // When resolve=false, bloc will return the transaction hash immeidately, and it is the caller's responibility to check it.
  if(resolve) {
    const resolvedResults = yield resolveResults(results, chainId, node);
...
*/
function* uploadContractString(user, contractName, contractSrc, args, options, doNotResolve, txParams, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  args = args || {};
  txParams = txParams || {};
  options = options || {};
  verbose('uploadContractString', {user, contractName, args, options, txParams, resolve, chainId, node});

  const metadata = constructMetadata(options, contractName);
...
function* uploadContract(user, contractName, contractFilename, args, options, doNotResolve, txParams, chainId, node) {
  verbose('uploadContract', {user, contractName, contractFilename, args, options, doNotResolve, txParams, chainId, node});
  // get the source
  const contractSrc = yield getContractString(contractName, contractFilename);
  // upload
  return yield uploadContractString(user, contractName, contractSrc, args, options, doNotResolve, txParams, chainId, node);
}
 ...
 */
function *  uploadContractList(user, txs, options, doNotResolve, chainId, node){
  const resolve = doNotResolve ? false : true;
  options = options || {};
  verbose('uploadContractList', {user, txs, resolve, chainId, node})
...
*/
function* send(fromUser, toUser, value, options, doNotResolve, nonce, chainId, node) {
  verbose('send', {fromUser, toUser, value, doNotResolve, chainId, node});
  const resolve = (doNotResolve) ? false : true;
  var result = yield api.bloc.send({
    ...
    }, fromUser.name, fromUser.address, resolve, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
...
function* sendList(fromUser, txs, options, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('sendList', {fromUser, txs, resolve, chainId, node});
...
function* getBlocResult(hash, resolve, chainId, node) {
  verbose('getBlocResult', {hash, resolve, chainId, node});
  const result = yield api.bloc.result(hash, resolve, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
function* getBlocResults(hashes, resolve, chainId, node) {
  verbose('getBlocResults', {hashes, resolve, chainId, node});
  const result = yield api.bloc.results(hashes, resolve, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
function* resolveResult(result, chainId, node) {
    return (yield resolveResults([result], chainId, node))[0];
}

function* resolveResults(results, chainId, node) {
    var count = 0;
    var res = results;
    while (count < 60 && res.filter(r => {return r.status === constants.PENDING}).length !== 0) {
        res = yield getBlocResults(res.map(r => {return r.hash}), chainId, false, node);
...
function* transactionResult(hash, chainId, node) {
  verbose('transactionResult', {hash, node});
  const result = yield api.strato.transactionResult(hash, chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
 ...
 */
 function* getAccount(address, chainIds, node) {
   verbose('getAccount', {address, chainIds, node});
   const accounts = yield api.strato.account(address, chainIds, node)
     .catch(function(e) {
       throw (e instanceof Error) ? e : new HttpError(e);
     });
 ...
 function* getBalance(address, accountIndex, chainId, node) {
   accountIndex = accountIndex || 0;
   verbose('getBalance', {address, accountIndex, chainId, node});
   const accounts = yield api.strato.account(address, chainId, node);
...
function* waitTransactionResult(hash, timeoutMilli, chainId, node) {
  ...
  for (var i = 0; i < retries ; i++) {
    // transactionResult
    var results = yield transactionResult(hash, chainId, node);
    // got one - done
    if (results.length > 0) {
      return results;
    }
 ...
 * @param{Boolean} doNotResolve tell bloc to wait for resolution or return immediately
 * @param{Object} txParams transaction params
 * @param{String} chainId
 * @param{Number} node target node
 * @returns{()} doNotResolve=true: transaction hash (String), doNotResolve=false: {status:String, hash:String, txResult:object, data: object}
*/
function* sendTransactions(token, txs, doNotResolve, txParams, chainId, node) {
  verbose('sendTransactions', {token, txs, doNotResolve, txParams, chainId, node});
  const resolve = (doNotResolve) ? false : true;
 ...
 * @returns{()} doNotResolve=true: [transaction hash] (String), doNotResolve=false: [method call return vals] (String|Int)
 */
function* callOAuth(token, contract, methodName, args, value, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('callOAuth', {token, contract, methodName, args, value, resolve, chainId, node});
  ...
  if(resolve) {
    ...
    const resolvedResults = yield resolveResult(result, chainId, node);
...
function* callMethodOAuth(token, contract, methodName, args, value, doNotResolve, chainId, node) {
  return yield callOAuth(token, contract, methodName, args, value, doNotResolve, chainId, node);
}
 ...
 * @returns{()} doNotResolve=true: transaction hash (String), doNotResolve=false: {status:String, hash:String, txResult:object, data: object}
 */
function* sendOAuth(token, toUser, value, doNotResolve, nonce, chainId, node) {
  verbose('sendOAuth', {token, toUser, value, doNotResolve, chainId, node});
  const resolve = (doNotResolve) ? false : true;
  const txs = [{
    payload: {
      toAddress: toUser.address,
...
function* sendListOAuth(token, txs, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('sendListOAuth', {token, txs, resolve, chainId, node});
  const newTxs = [];
  txs.forEach(function(tx) {
    newTxs.push({
      payload: tx,
...
function* callListOAuth(accessToken, txs, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('callListOAuth', {accessToken, txs, resolve, node});
 ...
 * @returns{()} doNotResolve=true: [transaction hash] (String), doNotResolve=false: [uploaded contract details]
*/
function* uploadContractStringOAuth(accessToken, contractName, contractSrc, args, doNotResolve, txParams, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  args = args || {};
  txParams = txParams || {};
  verbose('uploadContractStringOAuth', {accessToken, contractName, args, txParams, resolve, chainId, node});
...
function* uploadContractOAuth(accessToken, contractName, contractFilename, args, doNotResolve, txParams, chainId, node) {
  verbose('uploadContractOAuth', {accessToken, contractName, contractFilename, args, doNotResolve, txParams, chainId, node});
  // get the source
  const contractSrc = yield getContractString(contractName, contractFilename);
  // upload
  return yield uploadContractStringOAuth(accessToken, contractName, contractSrc, args, doNotResolve, txParams, chainId, node);
}
 ...
 * @param {number} node - Target node index
 */
function *  uploadContractListOAuth(accessToken, txs, doNotResolve, chainId, node){
  const resolve = doNotResolve ? false : true;
  verbose('uploadContractList', {accessToken, txs, resolve, chainId, node})
