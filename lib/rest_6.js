...
function verbose(prompt, args) {
  ...
  if( getLogger !== undefined) {
    const message = {prompt, args} // MUST be named `message`
...
class HttpError extends Error {
  constructor(e) {
    const data = (e.data) ? JSON.stringify(e.data).replace(/\\n/g, '\n').replace(/\\"/g, '"') : ''
    const url = (e.config && e.config.url) ? e.config.url : ''
    super(`${e.status} ${e.statusText}: ${data}: ${url}`);
    this.name = 'HttpError';
...
function* resolveResults(results, options={}) {
    ...
    const act = node => getBlocResults(results.map(r => r.hash), options)
    try {
 ...
 * @param{Object} options the optional arguments {node: integer}
 * @returns Address
 */
function* createKey(token, options={}) {
...
/**
 * Get the address of current user by token or other user by username provided in options
 * @method{getKey}
 * @param{String} token
 * @param{Object} options the optional arguments {username: string, node: integer}
 * @returns Address
 */
function* getKey(token, options={}) {
...
function* sendTransactions(token, txs, options={}) {
  ...
  var results = yield api.strato23.sendTransactions({
      ...
      'Authorization': 'Bearer ' + token
    }, !options.doNotResolve, options.chainId, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
