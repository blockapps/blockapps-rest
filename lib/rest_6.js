...
function* call(user, contract, methodName, args, options) {
  ...
  var result = yield api.bloc.call({
    ...
    }, user.name, user.address, contract.name, contract.address, resolve, options.chainId, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  ...
  if(resolve) {
    ...
    result = yield resolveResult(result, options.chainId, options.node);
...
function* callList(user, txs, options) {
  ...
  verbose('callList', {user, txs, options, resolve});
  ...
  const results = yield api.bloc.callList({
      ...
      txs: txs,
      resolve: resolve,
    }, user.name, user.address, resolve, options.chainId, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  // verity results
  ...
  // When resolve=false, bloc will return the transaction hash immeidately, and it is the caller's responibility to check it.
  if(resolve) {
    const resolvedResults = yield resolveResults(results, options.chainId, options.node);
...
function* uploadContractString(user, contractName, contractSrc, args, options) {
  ...
  verbose('uploadContractString', {user, contractName, args, options, resolve});
...
function *  uploadContractList(user, txs, options){
  options = options || {};
  const resolve = options.resolve;
  verbose('uploadContractList', {user, txs, options, resolve})
...
function* send(fromUser, toUser, value, options) {
  options = options || {};
  const resolve = options.resolve;
  verbose('send', {fromUser, toUser, value, options, resolve});
  ...
  var result = yield api.bloc.send({
    ...
    }, fromUser.name, fromUser.address, resolve, options.chainId, options.node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
...
function* sendList(fromUser, txs, options) {
  options = options || {};
  const resolve = options.resolve;
  verbose('sendList', {fromUser, txs, options, resolve});
...
function* callOAuth(token, contract, methodName, args, options) {
  ...
  const value = options.value || 0;
  const valueFixed = (value instanceof BigNumber) ? value.toFixed(0) : value;
  verbose('callOAuth', {token, contract, methodName, args, options, resolve});
  ...
  if(resolve) {
    ...
    const resolvedResults = yield resolveResult(result, options.chainId, options.node);
...
function* sendOAuth(token, toUser, value, options) {
  options = options || {};
  const resolve = options.resolve;
  verbose('sendOAuth', {token, toUser, value, options, resolve});
...
function* sendListOAuth(token, txs, options) {
  options = options || {};
  const resolve = options.resolve;
  verbose('sendListOAuth', {token, txs, options, resolve});
...
function* callListOAuth(accessToken, txs, options) {
  options = options || {};
  const resolve = options.resolve;
  verbose('callListOAuth', {accessToken, txs, options, resolve});
...
function* uploadContractStringOAuth(accessToken, contractName, contractSrc, args, options) {
  ...
  const resolve = options.resolve;
  args = options.args || {};
  txParams = options.xParams || {};
  verbose('uploadContractStringOAuth', {accessToken, contractName, args, options, resolve});
...
function *  uploadContractListOAuth(accessToken, txs, options){
  options = options || {};
  const resolve = options.resolve;
  verbose('uploadContractList', {accessToken, txs, options, resolve})
