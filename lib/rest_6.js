...
function* call(user, contract, methodName, args, value, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('call', {user, contract, methodName, args, value, resolve, chainId, node});
...
function* callMethod(user, contract, methodName, args, value, doNotResolve, chainId, node) {
  return yield call(user, contract, methodName, args, value, doNotResolve, chainId, node);
}
...
*/
function* callList(user, txs, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
...
*/
function* uploadContractString(user, contractName, contractSrc, args, doNotResolve, txParams, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
  args = args || {};
  txParams = txParams || {};
...
function* uploadContract(user, contractName, contractFilename, args, doNotResolve, txParams, chainId, node) {
  verbose('uploadContract', {user, contractName, contractFilename, args, doNotResolve, txParams, chainId, node});
  // get the source
  const contractSrc = yield getContractString(contractName, contractFilename);
  // upload
  return yield uploadContractString(user, contractName, contractSrc, args, doNotResolve, txParams, chainId, node);
}
 ...
 */
function *  uploadContractList(user, txs, doNotResolve, chainId, node){
  const resolve = doNotResolve ? false : true;
...
*/
function* send(fromUser, toUser, value, doNotResolve, nonce, chainId, node) {
...
function* sendList(fromUser, txs, doNotResolve, chainId, node) {
  const resolve = (doNotResolve) ? false : true;
 ...
 * @param{String} governance add rule
 * @param{String} governance remove rule
 * @param{Array} list of chain member enode addresses
 * @param{Array} list of account balances
 * @returns{()} ChainId
 */
function* createChain(label, members, balances, src, args, node) {
  verbose('createChain', {label, members, balances, src, args, node});
  const body = {
    label: label,
    members: members,
    balances: balances,
