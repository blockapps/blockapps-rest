...
function* createUser(name, password, node) {
  ...
  const address = yield api.bloc.createUser({
     password: password,
    }, name, isFaucet).catch(function(e){console.log('============================ promise rejection'); throw new Error('promise rejection')});
...
*/
function uploadContract(user, contractName, contractSrc, args, txParams, node) {
    args = args || {};
    txParams = txParams || {};
      verbose('uploadContract', {user, contractName, args, txParams, node});
      api.setNode(node);
      const address = api.bloc.contract({
          password: user.password,
          src: contractSrc,
          args: args,
