...
function* fill(user, resolve, node) {
  ...
  const blocResult = yield api.bloc.fill({ // FIXME - update to latest bloch
    ...
    }, user.address, user.name, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return blocResult;
...
function* call(user, contract, methodName, args, value, node) {
  ...
  if(result.status === constants.PENDING) {
    throw new HttpError202(methodName);
  }

  return result.txResult.data;
}
...
function* uploadContractString(user, contractName, contractSrc, args, txParams, node) {
  ...
  if(result.status === constants.PENDING) {
    throw new HttpError202(contractName);
  }

  const address = result.txResult.data[0];
...
function* send(fromUser, toUser, value, nonce, node) {
  ...
  if(result.status === constants.PENDING) {
    throw new HttpError202(`${fromUser} to ${toUser}: ${value}`);
  }
  return result.txResult.txReceipt;
}
