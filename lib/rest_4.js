...
*/
function* callList(user, address, txs, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
...
function* isCompiledByName(contractName) {
  try {
...
function* isCompiled(codeHash) {
  const results = yield rest.query(`contract?codeHash=eq.${codeHash}`);
