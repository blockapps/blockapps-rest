...
function* getChainInfo(chainId, node) {
  verbose('getChainInfo', {chainId, node});
  const chainInfo = yield api.strato.chain(chainId, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  ...
  chainInfo[0].accountBalance.forEach(function(ab){
    balances.push({address: ab[0], balance: ab[1]});
  });
  
  return {
    label: chainInfo[0].chainLabel
  , addRule: chainInfo[0].addRule
  , removeRule: chainInfo[0].removeRule
  , members: chainInfo[0].members
  , balances: balances
  };
}
...
function* createChain(label, addRule, removeRule, members, balances, node) {
  ...
  balances.forEach(function(b) {
    pairs.push([b.address,b.balance]);
  });
  const body = {
    chainLabel: label,
    addRule: addRule,
