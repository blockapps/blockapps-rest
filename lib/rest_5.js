...
function* getStateVar(contract, chainId, varName, varCount, varOffset, varLength, node) {
  verbose('getState', {contract, chainId, varName, varCount, varOffset, varLength, node});
  const state = yield api.bloc.stateVar(contract.name, contract.address, chainId, varName, varCount, varOffset, varLength, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return state;
...
function* call(user, contract, methodName, args, value, chainId, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('call', {user, contract, methodName, args, value, chainId, resolve, node});
  ...
  var result = yield api.bloc.call({
      ...
      args: args,
      value: valueFixed,
    }, user.name, user.address, contract.name, contract.address, chainId, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  ...
  if(resolve) {
    ...
    result = yield resolveResult(result, chainId);
...
function* callMethod(user, contract, methodName, args, value, chainId, doNotResolve, node) {
  return yield call(user, contract, methodName, args, value, chainId, doNotResolve, node);
}
...
*/
function* callList(user, txs, chainId, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
  ...
  const results = yield api.bloc.callList({
      ...
      txs: txs,
      resolve: resolve,
    }, user.name, user.address, chainId, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  // verity results
  ...
  // When resolve=false, bloc will return the transaction hash immeidately, and it is the caller's responibility to check it.
  if(resolve) {
    const resolvedResults = yield resolveResults(results, chainId);
...
*/
function* uploadContractString(user, contractName, contractSrc, args, chainId, doNotResolve, txParams, node) {
  const resolve = (doNotResolve) ? false : true;
  args = args || {};
  txParams = txParams || {};
  verbose('uploadContractString', {user, contractName, args, txParams, chainId, resolve, node});
...
function* uploadContract(user, contractName, contractFilename, args, chainId, doNotResolve, txParams, node) {
  verbose('uploadContract', {user, contractName, contractFilename, args, chainId, doNotResolve, txParams, node});
  // get the source
  const contractSrc = yield getContractString(contractName, contractFilename);
  // upload
  return yield uploadContractString(user, contractName, contractSrc, args, chainId, doNotResolve, txParams, node);
}
 ...
 */
function *  uploadContractList(user, txs, chainId, doNotResolve, node){
  const resolve = doNotResolve ? false : true;
  verbose('uploadContractList', {user, txs, chainId, resolve, node})
...
*/
function* send(fromUser, toUser, value, chainId, doNotResolve, nonce, node) {
  verbose('send', {fromUser, toUser, value, chainId, doNotResolve, node});
  const resolve = (doNotResolve) ? false : true;
  var result = yield api.bloc.send({
    ...
    }, fromUser.name, fromUser.address, chainId, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
...
function* sendList(fromUser, txs, chainId, doNotResolve, node) {
  const resolve = (doNotResolve) ? false : true;
  verbose('sendList', {fromUser, txs, chainId, resolve, node});
...
function* getBlocResult(hash, chainId, resolve, node) {
  verbose('getBlocResult', {hash, chainId, resolve, node});
  const result = yield api.bloc.result(hash, chainId, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
function* getBlocResults(hashes, chainId, resolve, node) {
  verbose('getBlocResults', {hashes, chainId, resolve, node});
  const result = yield api.bloc.results(hashes, chainId, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return result;
...
function* resolveResult(result, chainId) {
    var count = 0;
    var res = result;
    while (count < 60 && res.status === constants.PENDING) {
        res = yield getBlocResult(res.map(r => {return r.hash}), chainId, false);
...
function* resolveResults(results, chainId) {
    var count = 0;
    var res = results;
    while (count < 60 && res.filter(r => {return r.status === constants.PENDING}).length !== 0) {
        res = yield getBlocResults(res.map(r => {return r.hash}), chainId, false);
	yield promiseTimeout(1000);
	count++;
    }
...
function* waitTransactionResult(hash, chainId, timeoutMilli, node) {
...
/**
 * This function gets the chain details for a given chain Id
 * @method{getChainInfo}
 * @param{Array} chainIds
 * @param{Number} node target node
 * @returns{()} ChainInfo
 */
function* getChainInfo(chainIds, node) {
  verbose('getChainInfo', {chainIds, node});
  const chainInfo = yield api.strato.chain(chainIds, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  
  return {
    label: chainInfo[0][1].chainLabel
  , addRule: chainInfo[0][1].addRule
  , removeRule: chainInfo[0][1].removeRule
  , members: chainInfo[0][1].members
  , balances: chainInfo[0][1].accountBalance
  };
}
 ...
 * @returns{()} ChainId
 */
function* createChain(label, addRule, removeRule, members, balances, node) {
  verbose('createChain', {label, addRule, removeRule, members, balances, node});
  const body = {
    chainLabel: label,
    addRule: addRule,
  ...
  const chainId = yield api.strato.createChain(body, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return chainId;
