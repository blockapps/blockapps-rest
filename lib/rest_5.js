 ...
 * @returns Address
 */
function* createKey(token, isAsync, node) {
  verbose('createKey', {token, node});
  const address = yield api.strato23.createKey({}, node, {
  ...
  // otherwise - block for faucet fill call
  const txResult = yield api.strato.faucet({
    address: address
  }, true, node);
  return address;
}
 ...
 * @returns Address
 */
function* getKey(token, node) {
  verbose('getKey', {token, node});
  const address = yield api.strato23.key({}, node, {
    'Authorization': 'Bearer ' + token
  }).catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
 ...
 * @returns{()} doNotResolve=true: transaction hash (String), doNotResolve=false: {status:String, hash:String, txResult:object, data: object}
*/
function* sendTransactions(token, fromAddress, txs, doNotResolve, txParams, chainId, node) {
  verbose('sendTransactions', {token, fromAddress, txs, doNotResolve, txParams, chainId, node});
  const resolve = (doNotResolve) ? false : true;
...
// ========== OAuth =========
function oauthGetSigninURL() {
  return oauth.oauthGetSigninURL();
}

function* oauthGetAccessTokenByAuthCode(authCode) {
  verbose('oauthGetAccessTokenByAuthCode', authCode);
  
  const token = yield oauth.oauthGetAccessTokenByAuthCode(authCode).catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
  });
...
function* oauthRefreshToken(token) {
  yield oauth.oauthRefreshToken(token);
}
...
module.exports = {
  ...
  getNonce,
  // OAuth
  oauthGetSigninURL: oauthGetSigninURL,
  oauthGetAccessTokenByAuthCode: oauthGetAccessTokenByAuthCode,
