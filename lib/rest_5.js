...
function* getStateVar(contract, varName, varLength, varStart, node) {
  verbose('getState', {contract, varName, varLength, varStart, node});
  const state = yield api.bloc.stateVar(contract.name, contract.address, varName, varLength, varStart, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return state;
