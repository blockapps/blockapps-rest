...
function* resolveResults(results, chainId, node) {
    ...
    const act = n => getBlocResults(results.map(r => r.hash), chainId, false, n)
    try {
      return yield until(pred, act, undefined, node)
    } catch (e) {
      throw HttpError(`Transaction did not resolve: ${JSON.stringify(e)}`)
    }
}
