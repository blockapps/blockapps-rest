...
function* resolveResults(results) {
    const pred = res => !res.some(r => r.status === constants.PENDING)
    const act = node => getBlocResults(results.map(r => r.hash), false, node)
    const out = yield until(pred, act).catch(
      e => { throw new HttpError(`Transaction did not resolve: ${JSON.stringify(e)}`)})
    return out
}
...
function* until(predicate, action, timeoutMilli, node) {
  timeoutMilli = timeoutMilli || common.config.timeout || 60 * 1000;
  const phi = 1.618;
  let dt = 100;
  let totalSleep = 0;
