...
module.exports = function(config) {
  ...
  return {
    ...
    strato: {
      ...
      account: function(address, chainId, node) {
      ...
      storage: function(attr, value, chainId, node) {
        const query = util.buildQueryParams([util.toParam(chainId, "chainid=" + chainId), util.toParam(attr && value, attr+'='+value)]);
      ...
      chain: function(chainId, node) {
        const query = util.buildParam(chainId, "chainid=" + chainId);
        return ax.get(config.getStratoUrl(node), '/eth/v1.2/account' + query);
      },
      createChain: function(body, node) {
        return ax.post(config.getStratoUrl(node), body, '/chain');
      }
    },

    bloc: {
      contract: function(body, from, address, chainId, resolve, node) {
      ...
      uploadList: function(body, from, address, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
      ...
      call: function(body, name, address, contractName, contractAddress, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
      ...
      method: function(body, name, address, contractName, contractAddress, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.getBlocUrl(node), body, '/users/' + encodeURIComponent(name) + '/' + address + '/contract/' + contractName + '/' + contractAddress + '/call' + query);
      },
      callList: function(body, from, address, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.getBlocUrl(node), body, '/users/' + from + '/' + address + '/callList' + query);
      },
      send: function(body, from, address, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.getBlocUrl(node), body, '/users/' + from + '/' + address + '/send' + query);
      },
      sendList: function(body, from, address, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.getBlocUrl(node), body, '/users/' + from + '/' + address + '/sendList' + query);
      },
      result: function(hash, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.get(config.getBlocUrl(node), '/transactions/' + hash + '/result' + query);
      },
      results: function(hashes, chainId, resolve, node) {
        const query = chainResolveQuery(chainId, resolve);
      ...
      stateVar: function(name, address, chainId, varName, varCount, varOffset, varLength, node) {
        ...
        const pag = util.toParam(vName, vName + vCount + vOffset + vLength);
        const query = util.buildQueryParams([cid,pag]);
        return ax.get(config.getBlocUrl(node), '/contracts/' + name + '/' + address + '/state' + query);
      },
