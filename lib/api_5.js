  ...
  return {
    ...
    strato23: {
      createKey: function(body, node, headers) {
        return ax.post(config.getStrato23Url(node), body, '/key', headers);
      },
      key: function (body, node, headers) {
        return ax.get(config.getStrato23Url(node), body, '/key', headers);
      },
      sendTransactions: function(body, resolve, chainId, node, headers) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.getStrato23Url(node), body, '/transaction' + query, headers);
      }
    },
