  ...
  return {
    ...
    strato23: {
      createKey: function(body, node, headers) {
        return ax.post(config.STRATO_URL, body, '/strato/v2.3/key', headers);
      },
      key: function (node, headers) {
        return ax.get(config.STRATO_URL, '/strato/v2.3/key', headers);
      },
      sendTransactions: function(body, resolve, chainId, node, headers) {
        const query = chainResolveQuery(chainId, resolve);
        return ax.post(config.STRATO_URL, body, '/strato/v2.3/transaction' + query, headers);
      }
    },
