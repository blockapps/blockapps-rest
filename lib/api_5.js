...
module.exports = function(config) {
  ...
  return {
    ...
    bloc: {
      ...
      stateVar: function(name, address, varName, varLength, varStart, node) {
        vName = varName ? `?name=${varName}` : null;
        vLen = varLength ? `&length=${varLength}` : '';
        vStart = varStart ? `&start=${varStart}` : '';
        return ax.get(config.getBlocUrl(node), '/contracts/' + name + '/' + address + '/state' + (vName ? vName + vLen + vStart : ''));
      },
