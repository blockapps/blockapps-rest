...
async function resolveResults(pendingResults, _options = {}) {
  ...
  const action = async () =>
    getBlocResults(pendingResults.map(r => r.hash), options);
...
async function createContractAuth(user, contract, options) {
  const [pendingTxResult] = await api.createContractAuth(
    user,
    contract,
    options
  );
...
async function createContractListAuth(user, contract, options) {
  const [pendingTxResult] = await api.createContractListAuth(
    user,
    contract,
    options
  );
...
async function waitForAddress(contract, _options) {
  ...
  const results = await searchUntil(contract, predicate, options);
  return results[0];
}
