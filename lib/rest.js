...
function verbose(prompt, args) {
  if (common.config.apiDebug) {
    const string = (args===undefined) ? '' : stringifyArguments(args);
...
function stringifyArguments(args) {
  var string = '';
  for (arg in args) {
    //string += arg + ':' + JSON.stringify(arguments[arg], null, 2) + '\n';
    string += JSON.stringify(args[arg], null, 2) + '\n';
  }
  return string;
}
...
// pretty print
function pp() {
  console.log(stringifyArguments(arguments));
}
...
function getContractString(name, filename) {
  verbose('getContractString', arguments);
  return function(scope) {
    return new Promise(function(resolve, reject) {
      verbose('getContractString  file ', filename);
...
function uploadContract(userName, password, contractName, args, txParams, node) {
  verbose('uploadContract', arguments);
  return function(scope) {
      ...
      return api.bloc.sendList({
        ...
        .then(function(result) {
          ...
          if (txs.length != result.length)
            reject (new Error (`sendList: result size mismatch. Expected ${txs.length} actual ${result.length} `));
          // succcess
          resolve(scope);
        }).catch(errorify(reject));
    });
...
function uploadContractList(fromUser, contracts, txresolve, node) {
  verbose('uploadList', arguments);
  return function(scope) {
    const password = scope.users[fromUser].password;
    const fromAddress = scope.users[fromUser].address;
    return new Promise(function(resolve, reject) {
      ...
      return api.bloc.uploadList({
        ...
        .then(function(result) {
          verbose('uploadList: result', result);
          // store reslut
          var tx = {
            params: {fromUser, contracts, txresolve, node},
            result: result,
          };
          scope.tx.push(tx);
          // if sent / received mismatch
          if (contracts.length != result.length)
            reject (new Error (`uploadList: result size mismatch. Expected ${contracts.length} actual ${result.length} `));
          // succcess
          resolve(scope);
        }).catch(errorify(reject));
    });
