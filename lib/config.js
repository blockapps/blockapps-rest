...
// api urls
// TODO: rewrite in in API 7: only have the `nodeUrl` parameter in the config and concatenate each service's URI to it here
// TODO: refactor the url concatenation here, api_7.js, axios_wrapper.js
...
configFile.getSearchUrl = function (node) {
  if (node === undefined) node = 0;
  const configUrl = this.nodes[node].searchUrl;
  // The workaround to use blockapps-rest within the STRATO platform directly via docker network (backwards-compatible); don't blame me for concatenations, blame the other guy
  // using indexOf for better performance
  return (configUrl.indexOf('/cirrus') !== -1 && configUrl.indexOf('/search') === -1 ) 
      ? configUrl + '/search/' 
      : configUrl + '/'
}
configFile.getNodeUrl = function (node) {
  // A backward-compatibility for config structure oauth->stratoUrl
  if (this.oauth && this.oauth.stratoUrl) {
    return this.oauth.stratoUrl
  } else {
    if (node === undefined) node = 0;
    return this.nodes[node].nodeUrl;
  }
}
