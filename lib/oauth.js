...
module.exports = class OAuthInstance {
	 ...
	 * @returns {Boolean}
	 */
	async isTokenValid(accessToken, secret) {
		...
		switch(decoded.header.alg) {
			case 'RS256':
				...
				try {
					const verified = jwt.verify(accessToken, key, {ignoreExpiration: false} );
					return true
				} catch(e) {
					return false;
				}
			case 'HS256':
				try {
					const verified = jwt.verify(accessToken, secret, {ignoreExpiration: false} );
					return true;
				} catch(e) {
					return false;
				}
  ...
  async validateAndGetNewToken(req, res) {
    try {
      ...
      const tokenObject = {
			...
			const decoded = jwt.decode(tokenResponse.token['access_token']);

			req['cookies'][this.appTokenCookieName] = tokenResponse.token['access_token'];
			...
			res.cookie(this.appTokenCookieName, tokenResponse.token['access_token'], {
