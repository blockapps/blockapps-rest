	...
	*/
	async validateAndGetNewToken(req, res) {
        ...
        if (!accessTokenExpiry) {
          ...
          const expired = (expiry - unixTime(new Date())) <= 0;	// This checks if access token is completely expired
        ...
        const nearExpiration = (accessTokenExpiry - unixTime(new Date())) <= 300;	// If access token is about to expire in 300 seconds, get new access token in advance
