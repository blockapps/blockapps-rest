...
describe('state', function() {
  ...
  it('get state - large array', async () => {
    ...
    const filename = `${cwd}/lib/test/fixtures/LargeArray.sol`
  ...
  it('get state - getArray', async () => {
    ...
    const uid = util.uid()
    const constructorArgs = { size: SIZE }
    const contractArgs = await factory.createContractFromFile(`${cwd}/lib/test/fixtures/LargeArray.sol`, uid, constructorArgs)
    const contract = await rest.createContract(admin, contractArgs, options)
...
describe('call', function() {
  ...
  async function createContract(uid, admin, constructorArgs, options) {
    const filename = `${cwd}/lib/test/fixtures/CallMethod.sol`
    const contractArgs = await factory.createContractFromFile(filename, uid, constructorArgs)
    const contract = await rest.createContract(admin, contractArgs, options)
    return contract
  }
  ...
  it('call method', async () => {
    ...
    const callArgs = factory.createCallArgs(contract, methodArgs, method)
  ...
  it('call method with value', async () => {
    ...
    const callArgs = factory.createCallArgs(contract, methodArgs, method, value)
    const [result] = await rest.call(admin, callArgs, options)
    const expected = constructorArgs.var1 * methodArgs.var2
    assert.equal(result, expected, 'method call results')
  })
