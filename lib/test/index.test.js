...
import { uid as getUid, isAddress, isHash } from '../util'
...
describe('state', function() {
  ...
  it('get state - getArray', async () => {
    ...
    const uid = getUid()
    const constructorArgs = { size: SIZE }
    const contractArgs = await factory.createContractFromFile(`${fixtures}/LargeArray.sol`, uid, constructorArgs)
    const contract = await rest.createContract(admin, contractArgs, options)
...
describe('bloc user', function() {
  ...
  it('create bloc user', async () => {
    ...
    const isAddress = isAddress(user.address)
