...
describe('search until', function() {
  ...
  beforeEach(async () => {
    ...
    const contractArgs = await factory.createContractFromFile(filename, uid, { intValue: uid })
    contract = await rest.createContract(admin, contractArgs, options)
  })
...
describe('search query', function() {
  ...
  it('search multiple', async () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const contract = await createSearchContract(admin, uid, intValue, options)
    ...
    results.forEach((result, index) => {
      ...
      assert.equal(result.intValue, uid * 100 + index, 'address')
    })
  })
  ...
  it('search by value', async () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      assert.equal(result.intValue, uid * 100 + i, 'address')
    }
    // search by value
    for (let i = 0; i < count; i++) {
      const contract = contracts[i]
...
async function createSearchContract(admin, uid, intValue, options) {
  const filename = `${fixtures}/Search.sol`
  const contractArgs = await factory.createContractFromFile(filename, uid, { intValue })
