...
async function createSearchContract(admin, uid, index, options, specialCharacters = '') {
...
describe('search query', function() {
  ...
  it('search special characters', async () => {
    ...
    for (let i = 0; i < count; i++) {
      const contract = await createSearchContract(admin, uid, i, options, specialCharacters)
      contracts.push(contract)
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      assert.equal(result.address, contract.address, 'address')
      assert.equal(result.intValue, intValue(uid, i), 'intValue')
      assert.equal(result.stringValue, stringValue(uid, i)+specialCharacters, 'stringValue')
    }
  })
})
