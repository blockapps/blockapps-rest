...
contract ErrorCodes {
    ...
    it('parse enum - from file', async () => {
      const filename = `${cwd}/lib/util/test/fixtures/solidityParser/ErrorCodes.sol`
  ...
  describe('fields', () => {
    ...
    contract test {
      ...
      const fields = await solidityParser.parseFields(input)
    ...
    contract test {
      ...
      const fields = await solidityParser.parseFields(input, 'pre_')
    ...
    it('parse fields - from file', async () => {
      const filename = `${cwd}/lib/util/test/fixtures/solidityParser/FieldsTest.sol`
      const source = fsUtil.get(filename)
      const fields = await solidityParser.parseFields(source, 'PRE_')
