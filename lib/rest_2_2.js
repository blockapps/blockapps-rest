...
function* waitForPending(results) {  

  const seconds = 1 * 1000;
  var ret = results;
  while(anyPending(ret)) {
    promiseTimeout(seconds);
    ret = ret.map(function*(r) {
      return yield getBlocResult(r.hash, true, node)
      .catch(function(e) {
        throw (e instanceof Error) ? e : new HttpError(e);
      });
    });
...
function* call(user, contract, methodName, args, value, node) {
  ...
  result = waitForPending([result])[0];

  const RETURNS = 'returns';
  if (result.data.contents[RETURNS] === undefined) throw new Error('callMethodAddress: returns field missing: ' + JSON.stringify(result, null, 2));
  return result.data.contents[RETURNS];
}
...
function* callList(user, address, txs, node) {
  ...
  results = waitForPending(results);
