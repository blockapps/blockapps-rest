...
function* call(user, contract, methodName, args, value, node) {
  ...
  while(anyPending(result))
    {
      promiseTimeout(seconds);
      result = yield getBlocResult(result.hash, resolve, node)
      .catch(function(e) {
        throw (e instanceof Error) ? e : new HttpError(e);
      });
    }
  ...
  if (result.data[RETURNS] === undefined) throw new Error('callMethodAddress: returns field missing: ' + JSON.stringify(result, null, 2));
...
*/
function* callList(user, address, txs, node) {
  const resolve = true;
  ...
  var results = yield api.bloc.callList({
      ...
      txs: txs,
      resolve: resolve,
    }, user.name, address, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
...
function* send(fromUser, toUser, value, nonce, node) {
  const resolve = true;
  verbose('send', {fromUser, toUser, value, resolve, node});
...
function* sendList(fromUser, txs, node) {
  const resolve = true;
...
function* getBlocResult(hashes, resolve, node) {
  verbose('getBlocResult', {hashes, resolve, node});
  const results = yield api.bloc.result(hashes, resolve, node)
    .catch(function(e) {
      throw (e instanceof Error) ? e : new HttpError(e);
    });
  return results;
