...
const buildBytes = function(s,w) {
  if (s == 0) return 'ff' + w;
  const t = (255 - (s % 256)) % 16;
  const u = t.toString(16);
  return buildBytes(Math.floor(s / 16), u + w);
}
...
module.exports = {
  ...
  intToBytes32: function(int) {
    const big = new BigNumber(int);
    if (big < 0) {
      const string = buildBytes(-(big+1),'');
      return (Array(64).join('f')).slice(string.length - 1) + string
    }
    const string = big.toString(16)
