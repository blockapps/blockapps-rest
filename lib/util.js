...
function oauthGetSigninURL() {
  return common.oauth.oauthGetSigninURL();
}

function* oauthGetAccessTokenByAuthCode(authCode) {
  const token = yield common.oauth.oauthGetAccessTokenByAuthCode(authCode).catch(function(e) {
...
function* oauthRefreshToken(token) {
  yield common.oauth.oauthRefreshToken(token);
}
